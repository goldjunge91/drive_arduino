...
uint16_t ServoController::angleToPWM(float angle_rad) {
    ...
    return SERVO_PWM_MIN_US + static_cast<uint16_t>(normalized * (SERVO_PWM_MAX_US - SERVO_PWM_MIN_US));
}
...
ESCController::ESCController() : initialized_(false) {
...
ActuatorController::ActuatorController() : status_flags_(0) {}
...
float ActuatorController::clampAndFlag(float value, float min_val, float max_val, uint16_t flag_bit, uint16_t& flags) {
    if (value < min_val) {
        flags |= flag_bit;
        return min_val;
