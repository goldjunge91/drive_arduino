...
size_t FrameEncoder::encodeCommand(const CommandFramePayload& payload, uint8_t* buffer, size_t buffer_size) {
    return encodeFrame(FrameId::COMMAND, &payload, sizeof(payload), buffer, buffer_size);
}

size_t FrameEncoder::encodeState(const StateFramePayload& payload, uint8_t* buffer, size_t buffer_size) {
