...
static void process_command(char *line) {
    ...
    if (!strcmp(cmd, "help")) handle_help();
    else if (!strcmp(cmd, "x")) handle_stop();
    else if (!strcmp(cmd, "freq")) handle_freq(argc, argv);
    else if (!strcmp(cmd, "esc1") || !strcmp(cmd, "esc2")) handle_esc(argc, argv);
    else if (!strcmp(cmd, "gear")) handle_gear(argc, argv);
    else if (!strcmp(cmd, "encr")) handle_enc_read();
    else if (!strcmp(cmd, "encz")) handle_enc_reset();
    else if (!strcmp(cmd, "w") || !strcmp(cmd, "s") || !strcmp(cmd, "a") || !strcmp(cmd, "d") || !strcmp(cmd, "q") || !strcmp(cmd, "e")) handle_drive(cmd, argc, argv);
    else if (!strcmp(cmd, "m")) handle_motor(argc, argv);
    else {
...
static void handle_drive(const char* cmd, int argc, char *argv[]) {
    ...
    if (!strcmp(cmd, "w")) { motor_drive_pct(0, +s); motor_drive_pct(1, +s); motor_drive_pct(2, +s); motor_drive_pct(3, +s); }
    else if (!strcmp(cmd, "s")) { motor_drive_pct(0, -s); motor_drive_pct(1, -s); motor_drive_pct(2, -s); motor_drive_pct(3, -s); }
    else if (!strcmp(cmd, "a")) { motor_drive_pct(0, -s); motor_drive_pct(1, +s); motor_drive_pct(2, -s); motor_drive_pct(3, +s); }
    else if (!strcmp(cmd, "d")) { motor_drive_pct(0, +s); motor_drive_pct(1, -s); motor_drive_pct(2, +s); motor_drive_pct(3, -s); }
    else if (!strcmp(cmd, "q")) { motor_drive_pct(0, -s); motor_drive_pct(1, +s); motor_drive_pct(2, +s); motor_drive_pct(3, -s); }
    else if (!strcmp(cmd, "e")) { motor_drive_pct(0, +s); motor_drive_pct(1, -s); motor_drive_pct(2, -s); motor_drive_pct(3, +s); }
