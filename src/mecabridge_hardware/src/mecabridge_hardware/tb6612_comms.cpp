...
void TB6612Comms::setup(const std::string & serial_device, int32_t baud_rate, int32_t timeout_ms)
{
  ...
  if (port_to_use.empty()) {
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TB6612Comms"), "No device specified, attempting auto-detection...");
    try {
      port_to_use = findSerialPort("");
      RCLCPP_INFO(
        rclcpp::get_logger(
          "TB6612Comms"), "Auto-detected serial port: %s", port_to_use.c_str());
    ...
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TB6612Comms"), "Using specified serial port: %s", port_to_use.c_str());
  }
  ...
  if (timeout_ms <= 0) {
    RCLCPP_WARN(
      rclcpp::get_logger("TB6612Comms"), "Invalid timeout %d ms, using 50 ms",
      timeout_ms);
    timeout_ms = 50;
  }
  ...
  try {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("TB6612Comms"), "Port: %s, Baud: %d, Timeout: %d ms",
      port_to_use.c_str(), baud_rate, timeout_ms);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      rclcpp::get_logger("TB6612Comms"), "Failed to setup serial communication on port %s: %s",
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TB6612Comms"), "  - Incorrect port permissions (try: sudo chmod 666 %s)",
      port_to_use.c_str());
...
void TB6612Comms::setDifferentialMotors(int left_val, int right_val)
{
  ...
  try {
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger(
        "TB6612Comms"), "Differential motors: L=%d, R=%d", left_val, right_val);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TB6612Comms"), "Failed to send differential motor command (L=%d, R=%d): %s",
...
void TB6612Comms::setFourMotors(int fl, int fr, int rl, int rr)
{
  ...
  try {
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger(
        "TB6612Comms"), "Four motors: FL=%d, FR=%d, RL=%d, RR=%d", fl, fr, rl, rr);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TB6612Comms"), "Failed to send four motor command (FL=%d, FR=%d, RL=%d, RR=%d): %s",
      fl, fr, rl, rr, e.what());
    throw;  // Re-throw to allow caller to handle the error appropriately
  }
}
...
void TB6612Comms::readDifferentialEncoders(int & left_enc, int & right_enc)
{
  ...
  if (iss >> left_str >> right_str) {
    try {
      ...
      RCLCPP_DEBUG(
        rclcpp::get_logger(
          "TB6612Comms"), "Encoder values: L=%d, R=%d", left_enc, right_enc);
    } catch (const std::exception & e) {
      RCLCPP_WARN(
        rclcpp::get_logger("TB6612Comms"), "Failed to convert encoder values ('%s', '%s'): %s",
    ...
    RCLCPP_WARN(
      rclcpp::get_logger(
        "TB6612Comms"), "Invalid encoder response format: '%s' (expected: 'left right')",
      response.c_str());
  ...
  try {
    ...
    if (iss >> fl_str >> fr_str >> rl_str >> rr_str) {
      try {
        ...
        RCLCPP_DEBUG(
          rclcpp::get_logger(
            "TB6612Comms"), "Four encoder values: FL=%d, FR=%d, RL=%d, RR=%d", fl, fr, rl, rr);
      } catch (const std::exception & e) {
        RCLCPP_WARN(
          rclcpp::get_logger(
            "TB6612Comms"), "Failed to convert four encoder values ('%s', '%s', '%s', '%s'): %s",
      ...
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TB6612Comms"), "Invalid four encoder response format: '%s' (expected: 'fl fr rl rr')",
        response.c_str());
      fl = fr = rl = rr = 0;
    }
  } catch (const std::exception & e) {
    ...
    if (encoder_error_count % 20 == 1) {  // Log every 20 failures to avoid spam
      RCLCPP_ERROR(
        rclcpp::get_logger("TB6612Comms"), "Failed to read four encoders (error count: %d): %s",
...
std::string TB6612Comms::findSerialPort(const std::string & preferred_port)
{
  ...
  if (!preferred_port.empty()) {
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TB6612Comms"), "Testing preferred port: %s", preferred_port.c_str());
    try {
      ...
      if (test_conn.isOpen()) {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger(
            "TB6612Comms"), "Preferred port %s is available", preferred_port.c_str());
        return preferred_port;
      }
    } catch (const std::exception & e) {
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TB6612Comms"), "Preferred port %s failed: %s", preferred_port.c_str(), e.what());
  ...
  for (const auto & port : SERIAL_PORT_CANDIDATES) {
    try {
      ...
      if (test_conn.isOpen()) {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger(
            "TB6612Comms"), "Successfully found working port: %s", port.c_str());
        return port;
      }
    } catch (const std::exception & e) {
  ...
  RCLCPP_ERROR(
    rclcpp::get_logger(
      "TB6612Comms"), "No working serial port found after testing all candidates");
  ...
  RCLCPP_ERROR(
    rclcpp::get_logger(
      "TB6612Comms"), "  1. Check that the TB6612 device is connected and powered");
  RCLCPP_ERROR(rclcpp::get_logger("TB6612Comms"), "  2. Verify USB cable is functional");
  RCLCPP_ERROR(
    rclcpp::get_logger(
      "TB6612Comms"), "  3. Check port permissions: ls -l /dev/ttyUSB* /dev/ttyACM*");
  RCLCPP_ERROR(
    rclcpp::get_logger(
      "TB6612Comms"), "  4. Add user to dialout group: sudo usermod -a -G dialout $USER");
  RCLCPP_ERROR(
    rclcpp::get_logger(
      "TB6612Comms"), "  5. Specify device parameter explicitly if using non-standard port");
...
void TB6612Comms::sendMsg(const std::string & msg)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TB6612Comms"), "Serial write failed for message '%s' (error count: %d): %s",
...
std::string TB6612Comms::sendMsgWithResponse(const std::string & msg)
{
  ...
  try {
    ...
    if (response.empty()) {
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TB6612Comms"), "Empty response received for message: %s", msg.c_str());
    }
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("TB6612Comms"), "Message: %s -> Response: %s",
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TB6612Comms"), "Failed to send message '%s' with response (error count: %d): %s",
...
bool TB6612Comms::attemptReconnection()
{
  ...
  RCLCPP_WARN(
    rclcpp::get_logger(
      "TB6612Comms"), "Attempting to reconnect to TB6612 (attempt #%d)...", reconnection_attempts_);
...
void TB6612Comms::getConnectionStats(
