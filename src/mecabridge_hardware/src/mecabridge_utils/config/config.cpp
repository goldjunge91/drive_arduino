...
void Config::validate() const {
  ...
  if (state_publish_rate_hz < 10 || state_publish_rate_hz > 200) throw std::runtime_error("state_publish_rate_hz out of range");
  if (wheel_radius <= 0.0) throw std::runtime_error("wheel_radius must be >0");
  if (wheel_separation_x <= 0.0 || wheel_separation_y <= 0.0) throw std::runtime_error("wheel separation must be >0");
  ...
  // ESC ranges valid
  if (!(escs.left.esc_min_pwm < escs.left.esc_max_pwm) || !(escs.right.esc_min_pwm < escs.right.esc_max_pwm))
...
static inline int to_int(const std::string& v) {
  try { return std::stoi(v); } catch (...) { throw std::runtime_error("invalid integer for: " + v); }
}
static inline double to_double(const std::string& v) {
  try { return std::stod(v); } catch (...) { throw std::runtime_error("invalid float for: " + v); }
}
...
static void parse_inline_map(const std::string& expr, std::vector<std::pair<std::string,std::string>>& out) {
...
Config parse_from_yaml_string(const std::string& yaml) {
  ...
  while (std::getline(in, line)) {
    ...
    switch (sect) {
      case Sect::ROOT:
        if (key == "serial_port") cfg.serial_port = value;
        else if (key == "baud_rate") cfg.baud_rate = to_int(value);
        else if (key == "state_publish_rate_hz") cfg.state_publish_rate_hz = to_int(value);
        else if (key == "wheel_radius") cfg.wheel_radius = to_double(value);
        else if (key == "wheel_separation_x") cfg.wheel_separation_x = to_double(value);
        else if (key == "wheel_separation_y") cfg.wheel_separation_y = to_double(value);
        else if (key == "encoder_ticks_per_rev") cfg.encoder_ticks_per_rev = to_int(value);
        else if (key == "wheels") { /* ignore, handled by Sect::WHEELS */ }
        else if (key == "servos") { /* switch sect in following lines */ }
        else if (key == "escs") { /* switch sect */ }
        else if (key == "features") { /* switch sect */ }
        break;

      case Sect::WHEELS:
        if (key == "front_left" || key == "front_right" || key == "rear_left" || key == "rear_right") {
          std::vector<std::pair<std::string,std::string>> kv;
          parse_inline_map(value, kv);
          WheelDef* tgt = nullptr;
          if (key == "front_left") tgt = &cfg.wheels.front_left;
          else if (key == "front_right") tgt = &cfg.wheels.front_right;
          else if (key == "rear_left") tgt = &cfg.wheels.rear_left;
          else if (key == "rear_right") tgt = &cfg.wheels.rear_right;
          if (tgt) {
            for (auto& p : kv) {
              if (p.first == "encoder_index") tgt->encoder_index = to_int(p.second);
      ...
      case Sect::SERVOS_POS:
        if (key == "joint_name") cfg.servos.positional.joint_name = value;
        else if (key == "min_rad") cfg.servos.positional.min_rad = to_double(value);
      ...
      case Sect::SERVOS_CONT:
        if (key == "joint_name") cfg.servos.continuous.joint_name = value;
        else if (key == "max_velocity_rad_s") cfg.servos.continuous.max_velocity_rad_s = to_double(value);
        break;
      ...
      case Sect::ESCS:
        if (key == "left" || key == "right") {
          ...
          for (auto& p : kv) {
            if (p.first == "joint_name") tgt->joint_name = unquote(p.second);
            else if (p.first == "esc_min_pwm") tgt->esc_min_pwm = to_int(p.second);
            else if (p.first == "esc_max_pwm") tgt->esc_max_pwm = to_int(p.second);
      ...
      case Sect::FEATURES:
        if (key == "enable_servos") cfg.features.enable_servos = to_bool(value);
        else if (key == "enable_escs") cfg.features.enable_escs = to_bool(value);
        else if (key == "enable_diagnostics") cfg.features.enable_diagnostics = to_bool(value);
        break;
