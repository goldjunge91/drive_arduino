...
void TestableTB6612Comms::setup(
  ...
  int32_t timeout_ms)
{
  ...
  if (port_to_use.empty()) {
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "No device specified, attempting auto-detection...");
    try {
      port_to_use = findSerialPort("");
      RCLCPP_INFO(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Auto-detected serial port: %s", port_to_use.c_str());
    } catch (const std::exception & e) {
      RCLCPP_ERROR(
        rclcpp::get_logger("TestableTB6612Comms"), "Auto-detection failed: %s",
        e.what());
    ...
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Using specified serial port: %s", port_to_use.c_str());
  }
  ...
  if (baud_rate <= 0) {
    RCLCPP_WARN(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Invalid baud rate %d, using 115200", baud_rate);
    baud_rate = 115200;
  }
  ...
  if (timeout_ms <= 0) {
    RCLCPP_WARN(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Invalid timeout %d ms, using 50 ms", timeout_ms);
    timeout_ms = 50;
  }
  ...
  try {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Serial connection established successfully");

    // Wait for Arduino reset (matching Python behavior)
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Waiting for Arduino reset (2 seconds)...");
    std::this_thread::sleep_for(std::chrono::milliseconds(2000));

    // Send initial PING for synchronization
    RCLCPP_INFO(
      rclcpp::get_logger("TestableTB6612Comms"),
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("TestableTB6612Comms"), "Port: %s, Baud: %d, Timeout: %d ms",
      port_to_use.c_str(), baud_rate, timeout_ms);
  } catch (const std::exception & e) {
    is_connected_ = false;
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Failed to setup serial communication on port %s: %s",
...
void TestableTB6612Comms::sendPing()
{
  try {
    ...
    RCLCPP_WARN(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Arduino synchronization may be incomplete");
  }
}
...
void TestableTB6612Comms::setDifferentialMotors(int left_val, int right_val)
{
  ...
  try {
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Differential motors: L=%d, R=%d", left_val, right_val);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Failed to send differential motor command (L=%d, R=%d): %s",
...
void TestableTB6612Comms::setFourMotors(int fl, int fr, int rl, int rr)
{
  ...
  try {
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Four motors: FL=%d, FR=%d, RL=%d, RR=%d", fl, fr, rl, rr);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Failed to send four motor command (FL=%d, FR=%d, RL=%d, RR=%d): %s",
      fl, fr, rl, rr, e.what());
    throw;
  }
}
  ...
  try {
    ...
    if (iss >> left_str >> right_str) {
      try {
        ...
        RCLCPP_DEBUG(
          rclcpp::get_logger(
            "TestableTB6612Comms"), "Encoder values: L=%d, R=%d", left_enc, right_enc);
      } catch (const std::exception & e) {
        RCLCPP_WARN(
          rclcpp::get_logger(
            "TestableTB6612Comms"), "Failed to convert encoder values ('%s', '%s'): %s",
      ...
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Invalid encoder response format: '%s' (expected: 'left right')",
        response.c_str());
      left_enc = 0;
      right_enc = 0;
    }
    ...
    if (encoder_error_count % 20 == 1) {
      RCLCPP_ERROR(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Failed to read differential encoders (error count: %d): %s",
  ...
  try {
    ...
    if (iss >> fl_str >> fr_str >> rl_str >> rr_str) {
      try {
        ...
        RCLCPP_DEBUG(
          rclcpp::get_logger(
            "TestableTB6612Comms"), "Four encoder values: FL=%d, FR=%d, RL=%d, RR=%d", fl, fr, rl,
          rr);
      } catch (const std::exception & e) {
        RCLCPP_WARN(
          rclcpp::get_logger(
            "TestableTB6612Comms"), "Failed to convert four encoder values ('%s', '%s', '%s', '%s'): %s",
      ...
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Invalid four encoder response format: '%s' (expected: 'fl fr rl rr')",
        response.c_str());
      fl = fr = rl = rr = 0;
    }
  } catch (const std::exception & e) {
    ...
    if (encoder_error_count % 20 == 1) {
      RCLCPP_ERROR(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Failed to read four encoders (error count: %d): %s",
        encoder_error_count, e.what());
    }
    fl = fr = rl = rr = 0;
  }
...
std::string TestableTB6612Comms::findSerialPort(const std::string & preferred_port)
{
  ...
  if (!preferred_port.empty()) {
    RCLCPP_INFO(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Testing preferred port: %s", preferred_port.c_str());
    try {
      ...
      if (mock_serial_->isOpen()) {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger(
            "TestableTB6612Comms"), "Preferred port %s is available", preferred_port.c_str());
        return preferred_port;
      }
    } catch (const std::exception & e) {
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Preferred port %s failed: %s", preferred_port.c_str(), e.what());
  ...
  for (const auto & port : SERIAL_PORT_CANDIDATES) {
    try {
      ...
      if (mock_serial_->isOpen()) {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger(
            "TestableTB6612Comms"), "Successfully found working port: %s", port.c_str());
        return port;
      }
    } catch (const std::exception & e) {
      RCLCPP_DEBUG(
        rclcpp::get_logger("TestableTB6612Comms"), "Port %s failed: %s",
        port.c_str(), e.what());
  ...
  RCLCPP_ERROR(
    rclcpp::get_logger(
      "TestableTB6612Comms"), "No working serial port found after testing all candidates");
...
void TestableTB6612Comms::sendMsg(const std::string & msg)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Serial write failed for message '%s' (error count: %d): %s",
...
std::string TestableTB6612Comms::sendMsgWithResponse(const std::string & msg)
{
  ...
  try {
    ...
    if (response.empty()) {
      RCLCPP_WARN(
        rclcpp::get_logger(
          "TestableTB6612Comms"), "Empty response received for message: %s", msg.c_str());
    }
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("TestableTB6612Comms"), "Message: %s -> Response: %s",
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Failed to send message '%s' with response (error count: %d): %s",
...
bool TestableTB6612Comms::attemptReconnection()
{
  ...
  RCLCPP_WARN(
    rclcpp::get_logger(
      "TestableTB6612Comms"), "Attempting to reconnect to TB6612 (attempt #%d)...",
    reconnection_attempts_);
  ...
  try {
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestableTB6612Comms"), "Reconnection attempt failed: %s", e.what());
  }
...
void TestableTB6612Comms::getConnectionStats(
